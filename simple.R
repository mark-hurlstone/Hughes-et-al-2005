simple <- function(cellTotals,anovaTable,fixed,across){
  ...
  # 8. Calculate partial eta squared values
  #--------------------------------------------------------------------
  pes[1:acrossNl] = ss[1:acrossNl]/(ss[1:acrossNl]+ss[acrossNl+1])
  pes = round(pes, 3)
  
  # 9. Prepare the simple main effects table
  #--------------------------------------------------------------------
  seTable = data.frame(levels,ss,df,ms,f,p,pes)
